...
#--true-rttm-filename $true_rttm_filename
if [ $stage -le 0 ]; then
...
# ls $output_dir/rttm/*
mkdir -p $output_dir/rttm_$statScale
cp -r $output_dir/rttm/. $output_dir/rttm_$statScale
  # JOB=1
  #     /home/prachis/miniconda3/envs/py27/bin/python diarization/VB_resegmentation.py  --max-speakers $max_speakers \
  #       --max-iters $max_iters --downsample $downsample --alphaQInit $alphaQInit \
  # --sparsityThr $sparsityThr --epsilon $epsilon --minDur $minDur \
  # --loopProb $loopProb --statScale $statScale --llScale $llScale \
  # --channel $channel --initialize $initialize \
